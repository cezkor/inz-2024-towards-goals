...
class StepRangedDoubleQuestion(questionText: String, lower: Double, upper: Double, stepCount: Int)
    : RangedDoubleQuestion(questionText, lower, upper)
    {

        var stepCount: Int = stepCount
            private set
        init {
            if (stepCount < 1) this.stepCount = 1
            if (lower > upper) {
                val l = upper
                this.upper = lower
                this.lower = l
        ...
        var step: Double = (upper - lower)/stepCount
            private set
