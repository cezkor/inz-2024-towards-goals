...
import org.cezkor.towardsgoalsapp.etc.PermissionHelper
import org.cezkor.towardsgoalsapp.habits.questioning.HabitQuestioningContract
import org.cezkor.towardsgoalsapp.habits.questioning.HabitQuestions
...
class ReminderService : LifecycleService() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        val reminderNotificationChannel = NotificationChannel(
            org.cezkor.towardsgoalsapp.Constants.REMINDER_NOTIFICATION_CHANNEL,
        ...
        taskOrHabitOngoingLeft = object : BroadcastReceiver() {
            override fun onReceive(context: Context?, intent: Intent?) {
                ...
                lifecycleScope.launch {
                    if (remId != null && remId != org.cezkor.towardsgoalsapp.Constants.IGNORE_ID_AS_LONG) {
        ...
        val foregroundNotificationBuilder = Notification
            .Builder(this, org.cezkor.towardsgoalsapp.Constants.REMINDER_NOTIFICATION_CHANNEL)
    ...
    private suspend fun handleObsoleteReminder(reminder: ReminderData) = databaseMutex.withLock {
        if (reminder.ownerType == org.cezkor.towardsgoalsapp.OwnerType.TYPE_TASK) {
    ...
    private suspend fun handleReminderToFire(reminder: ReminderData) {
        ...
        val notificationBuilder = Notification
            .Builder(this@ReminderService, org.cezkor.towardsgoalsapp.Constants.REMINDER_NOTIFICATION_CHANNEL)
