...
class TextClassesTests {
    ...
    private fun testBehaviourOfNameFixer(tf: TextFixer, maxAllowedLength: Int) {
        ...
        assertThat(tf.fix(Constants.EMPTY_STRING)).isEqualTo(Constants.EMPTY_STRING)
        assertThat(tf.fix(null)).isEqualTo(Constants.EMPTY_STRING)
    ...
    fun `name fixers tests`() {
        ...
        testBehaviourOfNameFixer(NameFixer, Constants.NAME_LENGTH)
        testBehaviourOfNameFixer(ParamUnitFixer, Constants.UNIT_NAME_LENGTH)
        testBehaviourOfNameFixer(VeryShortNameFixer, Constants.VERY_SHORT_NAME_LENGTH)

        // test eisenhower matrix task name fixer
        // has to be tested separately as it appends ellipsis
        val maxAllowedLengthWithoutEllipsis = Constants.EISENHOWER_MATRIX_NAME_LENGTH
    ...
    fun `description fixer test`() {
        val maxLen = Constants.DESCRIPTION_LENGTH
        ...
        val maxAllowedLengthWithoutEllipsis = Constants.SHORTENED_DESCRIPTION_LENGTH
        val maxStringWithoutEllipsis = "a".repeat(maxAllowedLengthWithoutEllipsis)
        val maxStringWithoutEllipsisWithoutOne = "a".repeat(maxAllowedLengthWithoutEllipsis - 1)

        assertThat(maxAllowedLengthWithoutEllipsis).isGreaterThan(4)

        assertThat(tf2.fix(null)).isEqualTo(Constants.EMPTY_STRING)
        assertThat(tf2.fix(Constants.EMPTY_STRING)).isEqualTo(Constants.EMPTY_STRING)
